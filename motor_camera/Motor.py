...
class Motor:
    ...
    def macrostep(self, control_instance, n=1, cond = lambda x: True):
        curr_step = 0
        for i in range(n):
            ...
            if not self.ser is None:
                print("stepped!")
                curr_step += 1
                control_instance.update_progress(curr_step, self.total)
                self.ser.write('1'.encode())
                self.ser.write('\n'.encode())

    def fullRotation(self, control_instance, cond=lambda: True):  # Executes a full rotation of the motor
        self.macrostep(control_instance, self.total, cond)
